...
awk '/^import SwiftUI/ { next } /^private enum HomeTab/ { exit } { print }' "$ROOT/CalmIOS/HomeView.swift" > "$TEMP/models.swift"
awk '/^import SwiftUI/ { next } /^private struct ReturnHomeKey/ { exit } { print }' "$ROOT/CalmIOS/DiscoveryView.swift" >> "$TEMP/models.swift"
